<template>
	<app-layout>
		<template #header>
			<h1 class="font-semibold text-xl text-gray-800 leading-tight">Category List</h1>
		</template>
		...
		<div v-if="showModal" class="bg-slate-800 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
			<div class="bg-white px-16 py-14 rounded-md text-center">
				...
				<button @click="deleteRow(this.idCategory)" class="bg-red-500 px-7 py-2 ml-2 rounded-md text-md text-white font-semibold">Ok, Delete</button>
			</div>
		</div>
	...
	export default{
	    ...
	    form: {
	      ...
	      handler: throttle(function () {
	        this.$inertia.get(route('categories.index'), pickBy(this.form), { preserveState: true })
	      }, 150),
	    },
	  },
		methods: {
			...
			deleteRow(data) {
				this.$inertia.delete(route('categories.destroy', data));
				this.closeModal();
			},
			reset() {
    	...
    	sortTable(sortby){
    		...
    		this.$inertia.get(route('categories.index')+'?sortby='+sortby+'&direction='+this.direction);
